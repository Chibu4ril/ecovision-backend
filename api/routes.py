...
from eco_pipeline.main import run_pipeline

from .methods import fetch_unprocessed_files, delete_uploaded_files
...
async def run_prediction(request: PredictionRequest):
    try:
        ...
        logging.info(f"Running prediction for file: {selected_file}")

        # Directly run the Python function instead of spawning a subprocess
        result = run_pipeline(selected_file)
